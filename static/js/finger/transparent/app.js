    ...
    var TransparentTest = function (vertices, indices, texCoords, normals, texture) {
        ...
        var RunTransparent = function (vertexShaderText, fragmentShaderText, alp, childNumber, parent) {
            this.begin = function(canvas) {
                    var gl;
                    if(childNumber == 10){
                        canvas = util.getCanvas("can_aa");
                        gl = util.getGLAA(canvas);
                    }else gl = util.getGL(canvas);
                    var WebGL = true;
                    ...
                    glM.mat4.lookAt(viewMatrix, [0, 0, -7], [0, 0, 0], [0, 1, 0]);
                    ...
                    var loop = function () {
                        ...
                        glM.mat4.rotate(xRotationMatrix, identityMatrix, -1.5, [1, 0, 0]);
        ...
        this.begin = function (canvas, cb, level) {
            ...
            this.cb = cb;
            this.level = level;
            var root = '../../static/js/finger/transparent/'
            util.loadTextResource(root + 'shader.vs.glsl', function (vsErr, vsText, self) {
                if (vsErr) {
                    alert('Fatal error getting vertex shader (see console)');
                    console.error(vsErr);
                } else {
                    util.loadTextResource(root + 'shader.fs.glsl', function (fsErr, fsText, self) {
                        if (fsErr) {
